...
class Populator:
    ...
    def section61_loader(self) -> str:
        ...
        return (
            ...
            "Events supplied to epoching are the source trial triggers "
            "(`trial - 1`, converted from one-based indexing exactly once); "
            "the fixed decision window of 0.5-2.5 s post-cue is therefore "
            "2.5-4.5 s post-trigger, i.e., samples [s0+625, s0+1125] "
            "inclusive (501 samples) for a trigger at zero-based sample s0. "
            "Organizer artifact flags are read per trial from the source "
            "`artifacts` field and applied as an explicit exclusion mask; "
            "the validator requires exact set equality between flagged and "
            "excluded trial identifiers, identically across both decoders "
            "and all filter bands. Exactly 22 EEG channels enter the "
            "decoders; EOG channels are excluded."
        )
    ...
    def section61_cv(self) -> str:
        ...
        return (
            ...
            "JSON. A one-vs-rest CSP plus shrinkage LDA classifier selects "
    ...
    def section62_shell(self) -> str:
        ...
        return (
            ...
            "(flagged included); corrected MI moves by less than 0.05 bits "
            "for every subject, so no conclusion depends on the retention "
            "conditioning."
        )
    ...
    def section154(self) -> str:
        ...
        return (
            ...
            "intents, and no subject retains a qualifying intent at alpha = "
            ".90. No pooled operating-system claim is made, and no fold "
    ...
    def abstract_sentence(self) -> str:
        ...
        return (
            ...
            "lenient thresholds to none at F1 >= .90."
        )
