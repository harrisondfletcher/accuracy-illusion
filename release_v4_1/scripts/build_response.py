...
TMP=tempfile.TemporaryDirectory(prefix="accuracy-v41-text-")
W=Path(TMP.name)
for archive,dest in [("Accuracy_Illusion_Revision_v4_Package.zip","simulation"),("bci_20260909T031855Z.zip","bci")]:
    with zipfile.ZipFile(OUT/"scientific_archives"/archive) as z:
        for name in z.namelist():
            if name.endswith("/") or name.endswith(".zip") or "__MACOSX" in name or name.endswith(".DS_Store"): continue
            p=(W/dest/name).resolve()
            if not p.is_relative_to((W/dest).resolve()): raise ValueError("Unsafe archive member")
            p.parent.mkdir(parents=True,exist_ok=True);p.write_bytes(z.read(name))
SIM=W/'simulation/Accuracy_Illusion_Revision_v4' 
# Extra already-computed power tables that the manuscript refers to.
sfile=OUT/'Supplementary_Tables_S0_S8_v4_1_REVISED.md'
s=sfile.read_text()
def table(rows,header):return '\n'.join(['| '+' | '.join(header)+' |','|'+'|'.join(['---']*len(header))+'|']+['| '+' | '.join(row)+' |' for row in rows])
pu=json.loads((SIM/'results/power_u.json').read_text());pc=json.loads((SIM/'results/power_c2.json').read_text())
extra=[]
for label,pi in [('S7e','0.1'),('S7f','0.35')]:
 extra+=['## '+label+'. Uncertainty selectivity at error fraction '+pi,'', 'Each cell reports rejection probability [95% Wilson Monte Carlo interval], with 1,000 replications. Trial count is the total uncertainty-channel count. These are archived results newly displayed here, not rerun experiments.','']
 rows=[]
 for eff,cells in pu['cells'][pi].items():
  row=['0 (size)' if float(eff)==0 else eff]
  for n in ['100','200','500','1000']:
   c=cells[n];row.append(f'{c["power"]:.3f} [{c["ci"][0]:.3f}, {c["ci"][1]:.3f}]')
  rows.append(row)
 extra += [table(rows,['effect','100','200','500','1000']),'']
extra += ['## S7g. Presence contrast for compositional generalization','', 'Chance (1/12) versus .86, two-sided Fisher test. Each system receives the listed held-out trial count; intervals quantify Monte Carlo uncertainty across 1,000 replications.','']
rows=[]
for n,c in pc['presence'].items():rows.append([n,str(c['N_default_allocation']),f'{c["power"]:.3f}',f'[{c["ci"][0]:.3f}, {c["ci"][1]:.3f}]'])
extra += [table(rows,['Held-out trials per system','Implied default total N','Power','95% Monte Carlo interval']),'']
s=s.replace('## Table S8.','\n'.join(extra)+'\n## Table S8.',1)
sfile.write_text(s)
# Preserve original source outputs and actual scientific code without duplicate obsolete manuscripts.
for name in ['power_u.json','power_c2.json','power_r.json','power_bmin.json','grid.json','nonredundancy.json','counterexamples.json','null_models.json','sweep_noise.json','equal_mi_matrices.json']:
 shutil.copy2(SIM/'results'/name,OUT/'source_inputs/simulation'/name)
for name in ['power_v3.py','metrics_u.py','generate_worked_example.py']:
 shutil.copy2(SIM/name,OUT/'source_inputs/simulation'/name)
shutil.copytree(SIM/'data',OUT/'source_inputs/simulation/data',dirs_exist_ok=True)
shutil.copytree(SIM/'protocol',OUT/'source_inputs/simulation/protocol',dirs_exist_ok=True)
BCI=W/'bci/bci_20260909T031855Z/release_staging'
shutil.copytree(BCI/'code',OUT/'source_inputs/bci/executed_code',dirs_exist_ok=True)
shutil.copy2(BCI/'results/final_validation.json',OUT/'source_inputs/bci/final_validation.json')
# A readable source index gives exact locations independent of Word reflow.
m=(OUT/'The_Accuracy_Illusion_v4_1_REVISED.md').read_text()
def line_range(start,end=None):
 ls=m.splitlines();i=next(i for i,l in enumerate(ls) if start in l)
 if end:
  j=next(j for j in range(i+1,len(ls)) if end in ls[j])
 else:j=i+1
 return f'L{i+1}–L{j}'
def loc(section,start,end=None,sup=''):
 return f'**Location.** {section}; manuscript source {line_range(start,end)}'+ ('; '+sup if sup else '')+'.'
# Preserve actual prose comments in summarized headings; never invent a verbatim reviewer quotation.
r='''# Response to Reviewers — Revision 4.1
...
'''+loc('Sections 6.1–6.5','### 6.1','## 7.',sup='Tables 3a–3b; Supplementary S8a–S8i; executed BCI code')+'''
...
'''+loc('Sections 2.2, 6.2, and 7','### 6.2','### 6.3',sup='Table 3a; S8d, S8e, and S8h')+'''
...
'''+loc('Section 6.1','Episode duration is the full operating cycle.','### 6.2',sup='S8d and S8g; timing-audit JSON')+'''
...
'''+loc('Sections 3.4 and 4.2–4.3','### 4.2','## 5.',sup='Propositions 1, 3, and 4')+'''
...
'''+loc('Sections 5.7 and 7','### 5.7','### 5.8',sup='Tables 2a–2e; Figure 5; S7a–S7g')+'''
...
'''+loc('Sections 3.4–3.5, 6.4, 14, and 15.1–15.4','### 3.4','## 4.',sup='Propositions 3–4; Table 1a')+'''
...
'''+loc('Sections 2.6 and 5.5','### 2.6','### 2.7',sup='Table 1; S1b–S1c; core uncertainty CSV and reconstruction report')+'''
...
'''+loc('Sections 2.7 and 6.4','### 2.7','## 3.',sup='Table 0')+'''
...
'''+loc('Sections 5.2, 5.4, 5.7, and 6.2','### 5.2','### 5.5',sup='Tables 2a–2e and 3b; S8e')+'''
...
'''
(OUT/'Response_to_Reviewers_v4_1_REVISED.md').write_text(r)
for name in ['The_Accuracy_Illusion_v4_1_REVISED.md','Supplementary_Tables_S0_S8_v4_1_REVISED.md']:
 text=(OUT/name).read_text();index='\n'.join(f'{i:04d} | {line}' for i,line in enumerate(text.splitlines(),1))+'\n'
 idxname='The_Accuracy_Illusion_v4_1_SOURCE_LINES.txt' if name.startswith('The_') else 'Supplementary_Tables_v4_1_SOURCE_LINES.txt'
 (OUT/idxname).write_text(index)
print('Response words',len(r.split()),'Supplement',len(s.split()))
